                ...
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-100 ring-1 ring-black ring-opacity-5 py-2 transform origin-top-right transition-all animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="px-5 py-4 border-b border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`text-xs font-semibold ${
                          (user.storageUsed / user.maxStorage) * 100 > 90
                            ? "text-red-600"
                            : "text-blue-600"
                        }`}
                      >
                        {((user.storageUsed / user.maxStorage) * 100).toFixed(
                          0
                        )}
                        % used
                      </span>
                    ...
                    <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden mb-2">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ease-out ${getUsageColor(
                          (user.storageUsed / user.maxStorage) * 100
                        )}`}
                        style={{
                          width: `${(
                            (user.storageUsed / user.maxStorage) *
                            100
                          ).toFixed(0)}%`,
                        }}
                      />
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-gray-500">
                        <span className="font-semibold text-gray-900">
                          {renderFileSize(user.storageUsed)}
                        </span>{" "}
                        of {renderFileSize(user.maxStorage)}
                      </span>
                      <Link
                        to="/settings"
